...
const AI_MOODS = {
  neutral: {
    color: '#a1a1aa', label: 'Observing',
    eye: 'M30,50 L42,50 M58,50 L70,50',
    eyeWidth: 3,
  },
  thinking: {
    color: '#fbbf24', label: 'Calculating',
    eye: 'M30,50 L42,50 M58,50 L70,50',
    eyeWidth: 3,
  },
  confident: {
    color: '#a78bfa', label: 'Confident',
    eye: 'M30,53 L36,47 L42,53 M58,53 L64,47 L70,53',
    eyeWidth: 3,
  },
  aggressive: {
    color: '#f87171', label: 'Aggressive',
    eye: 'M30,46 L42,52 M58,52 L70,46',
    eyeWidth: 3,
  },
  worried: {
    color: '#60a5fa', label: 'Cautious',
    eye: 'M30,47 L36,53 L42,47 M58,47 L64,53 L70,47',
    eyeWidth: 3,
  },
  smug: {
    color: '#34d399', label: 'Smug',
    eye: 'M30,50 Q36,56 42,50 M58,50 Q64,56 70,50',
    eyeWidth: 2.5,
  },
};

function AIFace({ mood, speaking, size = 96 }) {
  const m = AI_MOODS[mood] || AI_MOODS.neutral;
  const color = m.color;
  const [eyePath, setEyePath] = React.useState(m.eye);

  // Soft transitions between mood eye shapes
  React.useEffect(() => {
    ...
    <div style={{ position: 'relative', width: size, height: size, flexShrink: 0 }}>
      ...
      <div
        style={{
          ...
          filter: `blur(8px)`,
          animation: mood === 'thinking' ? 'aiThinkPulse 1.6s ease-in-out infinite'
            : speaking ? 'aiSpeakPulse 0.8s ease-in-out infinite' : 'none',
          pointerEvents: 'none',
        }}
      />
...
function MoodChip({ label, color, active, value }) {
  return (
    <div
    ...
    >
      <div className="w-1 h-1 rounded-full" style={{ background: active ? color : '#3f3f46' }} />
      <span className="font-mono text-[9px] uppercase tracking-wider" style={{ color: active ? color : '#52525b' }}>
        {label}
      </span>
      <span className="font-mono text-[9px] ml-auto tabular-nums" style={{ color: active ? color : '#3f3f46' }}>
        {value}
      </span>
    </div>
  );
...
function SpeechBubble({ text, mood, speaking }) {
...
function AIPersona({ name, deck, mood, narration, speaking, confidence, intent, active, onTakeTurn, onExplain, taking, disabled, disabledReason }) {
  const m = AI_MOODS[mood] || AI_MOODS.neutral;
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: active ? `0 0 0 1px ${m.color}66, 0 8px 28px ${m.color}22` : '0 4px 16px rgba(0,0,0,0.4)',
      ...
      <div className="flex gap-2 p-4 pt-3">
        {isDisabled && !taking ? (
          ...
          >
            {taking ? (
              <>
                <span className="inline-block w-2 h-2 rounded-full" style={{ background: m.color, animation: 'aiTyping 1s infinite' }} />
                Thinking…
              </>
            ) : (
              <>
                <svg width="10" height="10" viewBox="0 0 10 10" fill="currentColor"><path d="M2 1l6 4-6 4z"/></svg>
                Take turn
              </>
            )}
          </button>
