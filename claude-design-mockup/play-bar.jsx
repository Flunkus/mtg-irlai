...
function PlayBar({ deck, onPlay, onDraw, onDiscard, onMulligan, lastPlayed }) {
  const [query, setQuery] = React.useState('');
  const [zone, setZone] = React.useState('battlefield');
  const [showMenu, setShowMenu] = React.useState(false);
  const [highlightIdx, setHighlightIdx] = React.useState(0);
  const inputRef = React.useRef(null);

  // De-duplicate by name and filter
  ...
  const playCard = (card) => {
    ...
    if (!data.name) return;
    onPlay({ ...data, id: 'play-' + Date.now() + '-' + Math.random().toString(36).slice(2, 7) }, zone);
    setQuery('');
    setShowMenu(false);
    setHighlightIdx(0);
  };
  ...
  const exactMatch = matches.find(m => m.name.toLowerCase() === trimmedQuery.toLowerCase());
  ...
  const handleKey = (e) => {
    if (e.key === 'ArrowDown') {
      ...
      e.preventDefault();
      if (hasMatches) {
        // Enter always picks the highlighted MATCH — never auto-falls through to freeform
        playCard(matches[highlightIdx]);
      } else if (trimmedQuery && e.shiftKey) {
        // Shift+Enter explicitly confirms freeform
        playCard(null);
      }
      // else: do nothing — make user explicitly confirm
  ...
  React.useEffect(() => { setHighlightIdx(0); }, [query]);
  ...
  return (
    ...
    >
      <div className="flex items-stretch gap-2 p-2">
        ...
        {/* Card name input + dropdown */}
        <div className="flex-1 relative min-w-0">
          <div className="relative h-full">
            <svg className="absolute left-2.5 top-1/2 -translate-y-1/2 text-zinc-500" width="13" height="13" viewBox="0 0 14 14" fill="none">
            ...
            <input
              ref={inputRef}
              value={query}
              onChange={e => { setQuery(e.target.value); setShowMenu(true); }}
              onKeyDown={handleKey}
              ...
              style={{
                borderColor: trimmedQuery && !hasMatches ? 'oklch(0.78 0.13 75 / 0.55)' :
                             trimmedQuery && hasMatches ? 'var(--accent)' : '#27272a',
              }}
            />
            <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1">
              {trimmedQuery && !hasMatches ? (
                <kbd className="font-mono text-[9px] uppercase tracking-wider px-1.5 py-0.5 rounded" style={{ background: 'oklch(0.78 0.13 75 / 0.18)', color: 'oklch(0.78 0.13 75)' }}>not found</kbd>
              ) : trimmedQuery && hasMatches ? (
                <kbd className="font-mono text-[9px] uppercase tracking-wider px-1.5 py-0.5 rounded" style={{ background: 'rgba(160,120,255,0.18)', color: 'var(--accent)' }}>↵ Add</kbd>
              ) : (
                <>
                  <kbd className="font-mono text-[9px] uppercase tracking-wider px-1.5 py-0.5 rounded bg-zinc-800 text-zinc-400">↵ Add</kbd>
                  <kbd className="font-mono text-[9px] uppercase tracking-wider px-1.5 py-0.5 rounded bg-zinc-800 text-zinc-400 hidden sm:inline">⇥ Zone</kbd>
                </>
              )}
            </div>
          </div>
              ...
              >
                <div className="px-2.5 py-1.5 flex items-center justify-between border-b border-zinc-800/70">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[9px] uppercase tracking-[0.16em] font-mono" style={{ color: 'var(--accent)' }}>
                ...
                {matches.map((m, i) => (
                  <div
                    key={m.id}
                    onMouseDown={(e) => { e.preventDefault(); playCard(m); }}
                  ...
                  >
                    ...
                    {m.pt && (
                      <span className="font-mono text-[10px] text-zinc-300 bg-zinc-900 px-1.5 py-0.5 rounded">{m.pt}</span>
                    )}
                  </div>
                ))}
              </div>
            ) : (
              // NO MATCHES — explicit freeform confirmation, different visual treatment
              <div
                className="absolute bottom-full left-0 right-0 mb-2 rounded-lg overflow-hidden z-30"
                style={{
              ...
              >
                <div className="px-3 py-2 border-b" style={{ borderColor: 'oklch(0.78 0.13 75 / 0.18)' }}>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-[9px] uppercase tracking-[0.16em] font-mono" style={{ color: 'oklch(0.78 0.13 75)' }}>
                      not found in your deck
                    </span>
                  </div>
                  <div className="text-zinc-300 text-[13px] leading-snug">
                    No card matching <span className="text-zinc-50 font-medium">"{trimmedQuery}"</span> in your active deck.
                  </div>
                </div>
                ...
                <div className="p-2 flex gap-2">
                  ...
                  <button
                    onMouseDown={(e) => { e.preventDefault(); playCard(null); }}
                ...
                <div className="px-3 py-1.5 border-t flex items-center gap-2 text-[10px] font-mono text-zinc-600" style={{ borderColor: 'oklch(0.78 0.13 75 / 0.12)' }}>
                  <span>Tip:</span>
        ...
        <div className="flex items-center gap-1 pl-2 border-l border-zinc-800">
          <ActionBtn label="Draw" onClick={onDraw} shortcut="D" />
          <ActionBtn label="Mill" onClick={() => onPlay({ id: 'mill-' + Date.now(), name: 'milled card', cost: '', type: 'Spell', pt: null, colors: ['C'] }, 'graveyard')} shortcut="M" />
          <ActionBtn label="Mulligan" onClick={onMulligan} shortcut="" />
        </div>
      </div>

      {/* Last played confirmation */}
      {lastPlayed && (
        <div
          key={lastPlayed.key}
        ...
        >
          <svg width="9" height="9" viewBox="0 0 10 10" fill="currentColor"><path d="M3.5 7.5L1.5 5.5l.7-.7 1.3 1.3 4.3-4.3.7.7z"/></svg>
...
function ActionBtn({ label, onClick, shortcut }) {
  return (
    <button
      onClick={onClick}
