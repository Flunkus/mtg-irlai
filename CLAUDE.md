...
- `src/components/`
  ...
  - `PlayBar.tsx` — "log a play" bar at the bottom of Battlefield. Has a `for: You/AI` side toggle; autocomplete switches deck based on side. Autocomplete matches by name OR collector number (numeric query, leading-zeros normalized, e.g. `297` → `0297`). The `send to` zone defaults from `activePhase` (Draw → Hand, Main → Battlefield); `Draw` focuses the input on the Hand zone so the drawn card is named/tracked.
  - `NewGameModal.tsx` — pick human deck, AI deck, life, who goes first, **and which AI persona to use**; dispatches `RESET_GAME`. (No hand-size field — the human logs their own opening hand.)
...
- `src/views/`
  - `Battlefield.tsx` — main play view. Reads `useGame()` for state, dispatches actions on tap/click/voice/brain-approval. Pulls active persona name + archetype + voice from `usePersonaLibrary()`; the local `speakOutLoud()` helper drives both the visual bubble and TTS audio. `AIDecisionPopup` shows the brain's proposal with **Reject / Question / Approve**; the **Question** button (shown only when an LLM key is set) opens quick-chip + free-text challenge UI that calls `proposeAIMove(..., challenge)` and replaces the proposal in place with a revised one. `SpellResolvePopup` shows the LLM's proposed resolution when the human casts an instant/sorcery (via `resolveSpell`), with Apply / Resolve-manually.
...
- `src/state/`
  - `gameStore.tsx` — Context + reducer for game state. `gameToCanonicalJson()` projects to the LLM-facing shape. `buildNewGameState()` constructs a fresh game from picked decks. `computeCombatResult()` is the pure damage-resolution helper used by the combat UI. `GameState.aiLandsPlayedThisTurn` tracks the AI's land drops (incremented on `PLACE_CARD` of an AI land to the battlefield, reset on turn change) so `manaModel` knows how many drops remain. **The human hand is tracked as real cards in `zones.hand` only — there is no untracked "ghost" count and no auto-dealt opening hand.** `buildNewGameState` starts both hands empty with a full library; the human logs each physical card (each `hand` add decrements the library). `Draw` names the drawn card; `Mulligan` shuffles the tracked hand back into the library.
...
- `src/llm/` — shared LLM layer used by **both** the AI brain and the voice parser.
  ...
  - `actionSchemas.ts` — `GameAction` discriminated union (tap/untap/play/move/adjust_life/declare_attackers) + `AIProposal` + `VoiceActions` + `SpellResolution` (human spell-resolver output: `summary`, `actions[]`, `needsManual`, `note`).
  - `applyActions.ts` — `applyActions(actions, state, dispatch)`: translates LLM-emitted actions into store dispatches, resolving card names to ids. Returns `{ applied, skipped }`. Also enforces the AI's one-land-per-turn drop as a backstop (seeded from `state.aiLandsPlayedThisTurn`).
  - `manaModel.ts` — deterministic resource model for the AI side. `parseManaCost`, `manaProduced`, `entersTapped`, `collectManaSources`, `canAfford`, and `buildResourceBlock(...)`. The brain is bad at counting, so we compute untapped mana / per-color availability / land-drop accounting / enters-tapped flags / per-spell castability HERE and feed it to the prompt as ground truth. This is the fix for the "pay 1W with one Plains / play two lands / turn-desync" hallucinations. Grounds the AI only — not a rules engine for the human.
  - `aiBrain.ts` — `proposeAIMove(state, persona?, challenge?)`: enriches AI hand + battlefield with Scryfall oracle text (module-level cache), injects the `manaModel` resource block + a RESOURCE RULES / SELF-CHECK section, builds a per-phase prompt, returns a parsed `AIProposal`. Uses Sonnet 4.6. When a `persona` is passed, its `personalityPrompt` is prepended to `SYSTEM_PROMPT` under a `=== PERSONA ===` block. When a `challenge` (`{ priorProposal, question }`) is passed — from the popup's **Question** button — the brain re-evaluates its last line against the human's challenge and returns a corrected or defended proposal.
  - `spellResolver.ts` — `resolveSpell(state, card)`: the human's mirror of the AI brain. When the human casts an instant/sorcery, reads its oracle text + board and returns a `SpellResolution` (destroy→move to graveyard, burn→adjust_life, bounce→move to hand, pump→add_counter, …). Reuses `structuredCall` + `applyActions`. Triggered from `Battlefield.playCardToZone` (which routes human instants/sorceries to the graveyard) and shown via `SpellResolvePopup`. Falls back to manual resolution when no key / on error.
- `src/voice/`
